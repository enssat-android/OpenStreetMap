...
import fr.enssat.osm.data.json.RandoResponse
...
interface DataApi {
    ...
    fun getRandos(@Query("\$select") select: String = fields.joinToString(separator = ",")): Call<RandoResponse>
}
...
class DataFetcher {
    ...
    fun fetchRandos(callback: (List<Rando>) -> Unit) {
        dataApi.getRandos().enqueue(object: Callback<RandoResponse> {
            override fun onResponse(call: Call<RandoResponse>, response: Response<RandoResponse>) {
            ...
            override fun onFailure(call: Call<RandoResponse>, t: Throwable) {
